...
function install_open_vm_tools {
    echoinfo "Installing Open VM Tools"
...
function install_vmware_tools {
    echoinfo "Installing VMware Tools"
    ...
    echoinfo "VMware Tools Path: ${VMWARE_TOOLS_PATH}"
    echoinfo "VMWare Tools Version: ${VMWARE_TOOLS_VERSION}"
    echoinfo "VMware Tools Build: ${VMWARE_TOOLS_BUILD}"
    ...
    echoingo "Installed VMware Tools ${VMWARE_TOOLBOX_CMD_VERSION}"
}

if [[ $PACKER_BUILDER_TYPE =~ vmware ]]; then
  KERNEL_VERSION=$(uname -r | cut -d. -f1-2)
  echoinfo "Kernel version ${KERNEL_VERSION}"
